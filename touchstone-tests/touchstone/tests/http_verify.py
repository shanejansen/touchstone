...
class GetCalled(TouchstoneTest):
    ...
    def when(self, given) -> object:
        urllib.request.urlopen(f'{self.mocks.http.network.url()}/some-endpoint')
        return None
...
class PostCalled(TouchstoneTest):
    ...
    def when(self, given) -> object:
        urllib.request.urlopen(f'{self.mocks.http.network.url()}/some-endpoint', data=bytes())
        return None
...
class PutCalled(TouchstoneTest):
    ...
    def when(self, given) -> object:
        request = urllib.request.Request(f'{self.mocks.http.network.url()}/some-endpoint', method='PUT')
...
class DeleteCalled(TouchstoneTest):
    ...
    def when(self, given) -> object:
        request = urllib.request.Request(f'{self.mocks.http.network.url()}/some-endpoint', method='DELETE')
        urllib.request.urlopen(request)
        return None
...
class PostContained(TouchstoneTest):
    ...
    def when(self, given) -> object:
        ...
        request = urllib.request.Request(f'{self.mocks.http.network.url()}/some-endpoint', method='POST', data=body)
        urllib.request.urlopen(request)
        return None
...
class PutContained(TouchstoneTest):
    ...
    def when(self, given) -> object:
        body = 'foo'.encode('utf8')
        request = urllib.request.Request(f'{self.mocks.http.network.url()}/some-endpoint', method='PUT', data=body)
        urllib.request.urlopen(request)
        return None
